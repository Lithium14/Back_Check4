...
export class AuthService {
  ...
  async signin(email: string, password: string) {
    ...
    const secret1 = environment.JWT_SECRET; // Variable environnement
    if (!secret1) {
  ...
  private async nodemailer(token: string, user: User) {
    ...
    try {
      ...
      const info = await transporter.sendMail({
        from: environment.EMAIL, // sender address // Mettre adresse client
        to: user.email, // list of receivers
        subject: 'Activation link', // Subject line
        html: `<b><a href=${environment.confirmationUrl + token}>
        Activation link </a>
        </b>`, // html body
      });
